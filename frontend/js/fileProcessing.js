 ...
 */
function handleFile(file) { // 保持 export
    // disclaimerAccepted 现在由 main_index.js 或其他页面脚本在 window 上管理，或通过参数传递。
    // 在这里我们仍然通过 window 访问，因为它是 UI 状态的一部分，由其他组件（如 main_index.js 的事件监听器）更新。
    // 如果 main_index.js 不再将 disclaimerAccepted 挂载到 window，这里需要调整。
    // 假设 main_index.js 会将其作为 window.disclaimerAccepted 挂载。
    ...
    showStatus('正在处理文件...', 'text-blue-500'); // 直接调用导入的 showStatus
    // 清理UI和现有数据，准备处理新文件 (注意：这里清空的是前端临时数据，后端会做追加)
    // window.tableData = []; // 不再立即清空全局 tableData，因为我们要追加
    // window.displayedTableData = []; // 这个也会在 applyFiltersAndSort 中重新计算
    ...
    if (fileName.endsWith('.csv')) {
        if (typeof Papa === 'undefined') {
             showStatus('错误: CSV解析库(PapaParse)未能加载。', 'text-red-500', 5000);
             return;
        }
        Papa.parse(file, {
            header: true,
            skipEmptyLines: true,
            complete: (results) => parseComplete(results.data, results.meta.fields || [], [], 'CSV'),
            error: (error) => parseError(error, 'CSV')
        });
    } else if (fileName.endsWith('.xls') || fileName.endsWith('.xlsx')) {
        if (typeof XLSX === 'undefined') {
            showStatus('错误: Excel解析库(XLSX)未能加载。', 'text-red-500', 5000);
            return;
        }
        reader.onload = (e) => {
            try {
        ...
        reader.onerror = (error) => parseError(error, 'FileReader (Excel)');
        reader.readAsBinaryString(file);
    } else {
        showStatus('不支持的文件类型。请上传CSV, XLS, 或 XLSX 文件。', 'text-red-500', 4000);
    }
}
...
/**
 * 文件解析成功后的回调处理函数。
 * @param {Array<object>} parsedDataFromFile 解析出的数据行对象数组。
 * @param {Array<string>} headers 文件中的原始表头数组。
 * @param {Array<object>} errors 解析过程中可能出现的错误。
 * @param {string} type 文件类型 ('CSV' 或 'Excel')。
 */
async function parseComplete(parsedDataFromFile, headers, errors, type) {
    if (errors && errors.length > 0) {
        console.error(`fileProcessing.js/${type} Parsing Errors:`, errors);
        showStatus(`解析${type}文件时发生错误: ${errors[0].message || '未知解析错误'}.`, 'text-red-500', 5000);
        return;
    }

    if (!parsedDataFromFile || parsedDataFromFile.length === 0) {
        showStatus(`${type}文件为空或无有效数据。`, 'text-yellow-500', 3000);
         // 不需要清空 window.tableData，因为我们是追加模式
        return;
    }

    // 将解析的数据转换为内部 tableData 格式，准备发送给后端
    // 后端会处理去重和分配 db_id
    const newEntriesToSend = parsedDataFromFile.map((row, index) => ({
        ...row,
        _id: `new-${Date.now()}-${index}-${Math.random().toString(36).substring(2, 7)}`, // 临时前端ID
        pdfLink: row.pdfLink || '',
        status: row.status || '待处理',
        screenshots: (row.screenshots && Array.isArray(row.screenshots)) ? row.screenshots.map(ss => ({ ...ss })) : [],
        localPdfFileObject: null
    }));

    showStatus(`成功从${type}文件解析了 ${newEntriesToSend.length} 条记录。正在尝试同步到服务器...`, 'text-blue-500');

    let syncResponse = null;
    if (typeof saveFullLiteratureList === "function") {
        syncResponse = await saveFullLiteratureList(newEntriesToSend); // 发送新解析的条目
    } else {
        console.error("fileProcessing.js/parseComplete: saveFullLiteratureList function not defined.");
        showStatus("错误：API服务未就绪，无法保存列表到服务器。", "text-red-500", 5000);
         // 即使API调用失败，也不应直接修改全局 window.tableData，因为我们依赖服务器做合并
        return; // 提前退出，不刷新本地列表，等待用户修复API问题或重试
    }

    if (syncResponse && syncResponse.success) {
        // 使用后端返回的更详细的消息
        let serverMessage = syncResponse.message || `服务器处理完成：新增 ${syncResponse.added || 0} 条，跳过 ${syncResponse.skipped || 0} 条。正在刷新本地列表...`;
        showStatus(serverMessage, 'text-blue-500', 6000);

        // 关键：同步成功后，从服务器重新加载完整的、最新的列表
        // 这将包含所有累计的、去重后的记录，以及由后端分配的 db_id
        ...
        if (serverDataLoaded) {
             showStatus('文献列表已更新并显示。', 'text-green-500', 3000);
        } else {
            showStatus('从服务器刷新列表失败，请手动刷新或检查网络。', 'text-orange-500', 5000);
        }

    } else {
        // 同步到服务器失败或部分失败
        let errorMessage = '同步到服务器失败。';
        if (syncResponse && syncResponse.message) {
            errorMessage = syncResponse.message;
        }
        showStatus(`${errorMessage} 请检查网络或服务器状态。解析出的数据未添加到您的记录中。`, 'text-red-500', 7000);
        // 由于是追加模式，如果同步失败，我们不应该修改现有的 window.tableData 或 localStorage
        // 用户需要解决问题后重新上传文件
    }
}
...
function parseError(error, type) {
    ...
    showStatus(`读取${type}文件失败: ${error.message || '未知错误，请检查文件格式和内容。'}.`, 'text-red-500', 7000);
}
...
function downloadUploadTemplate() {
    ...
    if (!COLUMN_MAPPING) { // <--- 直接使用导入的 COLUMN_MAPPING
        ...
        showStatus("无法下载模板：配置信息缺失。", "text-red-500", 3000);
        return;
    }
    ...
    if (link.download !== undefined) {
        ...
        showStatus('上传模板文件已开始下载。', 'text-green-500', 3000);
    } else {
        alert('您的浏览器不支持直接下载。请手动复制以下表头并创建CSV文件：\n\n' + headerRowString);
        showStatus('无法直接下载模板，请手动复制表头。', 'text-yellow-500', 5000);
    }
}
